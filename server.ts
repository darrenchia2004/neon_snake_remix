...
io.on('connection', (socket) => {
  ...
  socket.on('update_state', (data: { segments: any[], score: number, streak: number, lastOrbColor: string | null, glow: number, currentAngle: number, isBoosting: boolean, state: string }) => {
    const player = state.players[socket.id];
    if (player && player.state === 'alive') {
      player.segments = data.segments;
      player.score = data.score;
      player.streak = data.streak;
      player.lastOrbColor = data.lastOrbColor;
      player.glow = data.glow;
