...
function Snake({ playerId, color, isLocal }: { playerId: string, color: string, isLocal: boolean }) {
  ...
  useFrame((state, delta) => {
    ...
    if (headMatRef.current) {
      headMatRef.current.emissiveIntensity = 1 + (player.glow || 0) * 5;
    }
    if (bodyMatRef.current) {
      bodyMatRef.current.emissiveIntensity = 1 + (player.glow || 0) * 2;
    }
        ...
        <meshStandardMaterial
          ...
          onBeforeCompile={(shader) => {
            shader.fragmentShader = shader.fragmentShader.replace(
              ...
              totalEmissiveRadiance += diffuseColor.rgb * (0.1 + fresnel * 3.0);
              `
            );
          }}
        />
        ...
        <meshStandardMaterial
          ...
          onBeforeCompile={(shader) => {
            shader.fragmentShader = shader.fragmentShader.replace(
              ...
              #include <emissivemap_fragment>
              float fresnel = pow(1.0 - max(dot(normal, normalize(vViewPosition)), 0.0), 2.0);
              totalEmissiveRadiance += diffuseColor.rgb * (0.1 + fresnel * 1.5);
              `
            );
          }}
        />
...
function CollectionEffect({ x, y, color, onComplete }: { x: number, y: number, color: string, onComplete: () => void }) {
  ...
  return (
    <mesh ref={meshRef} position={[x, y, 0.6]}>
  ...
  useFrame((state, delta) => {
    ...
    if (serverPlayer && serverPlayer.state === 'alive') {
      ...
      if (!localPlayerRef.current.active && serverPlayer.segments.length > 0) {
        ...
        localPlayerRef.current.streak = serverPlayer.streak || 0;
      ...
      // Glow decay
      if (localPlayerRef.current.glow > 0) {
        localPlayerRef.current.glow -= delta * 2;
        if (localPlayerRef.current.glow < 0) localPlayerRef.current.glow = 0;
      }
      ...
      for (const orbId in gs.orbs) {
        ...
        if (dx * dx + dy * dy < 4) {
          // Streak logic
          if (orb.color === localPlayerRef.current.lastOrbColor) {
            localPlayerRef.current.streak++;
          } else {
          ...
          // Add effect
          setEffects(prev => [...prev, { id: orbId, x: orb.x, y: orb.y, color: orb.color }]);
      ...
      gs.players[playerId].streak = localPlayerRef.current.streak;
      gs.players[playerId].glow = localPlayerRef.current.glow;
      gs.players[playerId].lastOrbColor = localPlayerRef.current.lastOrbColor;
      ...
      if (now - localPlayerRef.current.lastSendTime > 50) {
        sendPlayerState({
          segments: localPlayerRef.current.segments,
          score: localPlayerRef.current.score,
          streak: localPlayerRef.current.streak,
          lastOrbColor: localPlayerRef.current.lastOrbColor,
          glow: localPlayerRef.current.glow,
          currentAngle: localPlayerRef.current.currentAngle,
          isBoosting: localPlayerRef.current.isBoosting,
          state: 'alive'
        });
